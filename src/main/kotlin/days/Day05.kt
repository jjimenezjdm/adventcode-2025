...
object Day05 {
    ...
    private fun solvePart1(input: List<String>): Int {
        ...
        val ranges = input.subList(0, blankLineIndex)
            .map { line ->
                val (start, end) = line.split("-").map { it.toLong() }
                start..end
            }
        
        // Parse available IDs (second part, after blank line)
        val availableIds = input.subList(blankLineIndex + 1, input.size)
            .map { it.toLong() }
        
        // Count how many IDs are fresh (in any range)
        var freshCount = 0
